...
**ABI version: `DCU_ABI_VERSION`, defined in [`dcu.h`](../native/dcu/include/dcu.h) — deliberately not restated here.** Version 1 was the pre-hardening surface and the version-2 changes were breaking; the package is unreleased, so there is no compatibility shim at any version. The exported set is enumerated below and diffed against `native/exports/expected-symbols.txt` (§11).

> This sentence used to carry two literals, `2` and `19`, and **both had drifted** — the header was at 3 and the list at 21. It is the same defect as the *Exported surface* heading below, which was fixed in [#123](https://github.com/xuhuanhello/juice-c-sharp/issues/123) while this line was missed one paragraph away. `docs/verification-mcp.md` §4 already tells its reader to compare against the header rather than a number in prose; a spec that then prints the number anyway hands them a second, unchecked source to be wrong.
...
**Common symbols were the one thing narrowing could not reach, and that hole is now closed** ([#108](https://github.com/xuhuanhello/juice-c-sharp/issues/108)). `-exported_symbols_list` can only demote symbols that have a section, and a tentative definition compiled as common has none; Apple clang defaults C to `-fcommon`, so 11 of them survived as external `C` in the first landed archive. `native/CMakeLists.txt` now carries **`-fno-common` as a global convention for `native/` C code** — not an iOS branch, because it describes how this project's C compiles, and iOS is merely the only platform that *exposes* it (a static archive hands its symbols to the adopter's final link; a dynamic library resolves its own commons at link time). The definitions get a section, and the existing `ld -r` step localises them. Measured: `C _foo` → `S _foo` → `s _foo`, and on the rebuilt archive all 11 are local `s`, with **external defined down to exactly the 20 `dcu_*`**.

Why no upstream patch was needed: all 11 are ordinary single-translation-unit tentative definitions with `extern` in a header (ten from usrsctp, one `_mbedtls_cipher_supported`), so they never relied on cross-TU common merging and `-fno-common` cannot produce a duplicate definition. Desktop regression confirms the convention is inert where it was already the default: the macOS `.dylib` rebuilt with it has a **byte-identical export set and dependency set** on both architectures.
...
- **`exports_ios` reads type `C` (common), because common is external defined too** ([#104](https://github.com/xuhuanhello/juice-c-sharp/issues/104), [#108](https://github.com/xuhuanhello/juice-c-sharp/issues/108)). It once read only `T/W/D/B`, and 11 external commons walked past the strict export-set diff: ten from usrsctp, one `_mbedtls_cipher_supported`, including an orphan `_foo` in usrsctp's `user_recv_thread.c`. They are visible to adopters in the worst way — a common merges silently with a same-named common in the final link, sharing one storage location, with no error and no warning.
  **The rule this gate enforces was never "only `dcu_*` text symbols" — it is "the external defined set equals `expected-symbols.txt`"**. So the fix was neither a common allowlist nor a fourth gate: `-fno-common` became a global convention for `native/` C code (§9), tentative definitions got a section, and the existing narrowing step localised them. `exports_ios` reading `C` is what makes a *future* common hit the wall instead of slipping past — the criterion sees it, rather than trusting a compiler's default. The by-name crypto exemption for `_mbedtls_cipher_supported` is gone with it, since no external common should exist any more; keeping it would have left a permanent silent channel for "common came back". Measured: the pre-fix archive reports **31** external defined (11 extra), the rebuilt one **exactly 20** with nothing extra and nothing missing.
- **The third gate is the one the other two structurally cannot replace.** An archive holding only the wrapper exports the full `dcu_*` set and leaks no crypto name — full marks on both — while missing every dependency; the failure lands in the adopter's Xcode link, not in our CI. The same assertion also runs inside `narrow_ios_archive.py` at build time, because failing during the build is cheaper than failing at the landing gate.
...
> **All nine steps have landed.** This list is kept as the record of the order they were done in — and of the one constraint that had to come first — not as a to-do list. ~~The scaffold, the CMake build, the desktop plugin and a first pass at the event pump and ICE marshalling already exist. What remains is the hardening described in §4/§6/§7 — and the first move is not any of it.~~
>
> Read each step below as "this is in place", with the parenthetical marking how it was verified. The normative ordering of 1–3 still matters: it is why the handle table was never written twice (§2).

1. **Migrate `dcu_impl.cpp` to the libdatachannel C++ API** (§2), preserving the three invariants listed there. Behaviour must not change: the pre-migration baseline is recorded in [#42](https://github.com/xuhuanhello/juice-c-sharp/issues/42) (offline audit, EditMode green, native create/dispose, dual-peer loopback, zero Console errors) and must be reproduced afterwards. **Do not** build the handle table against the C API first as a stepping stone — that writes it twice and defers exactly the hard part (handle allocation and liveness, type discrimination, cross-thread table locking) that the C API is currently doing for us. **Done** — `dcu_impl.cpp` carries no `rtc/rtc.h` include and no `rtc*` C entry point; the C++ API is what it calls.
2. **Rewrite the ABI surface** (§4): `dcu_event_next`, `dcu_dc_receive`, `dcu_dc_state`, `dcu_log_next`, `dcu_event_queue_depth`; return-code + out-param throughout; independent error numbering; explicit state mapping; `DCU_ABI_VERSION` → 2; update `native/exports/expected-symbols.txt` in the same commit. **Done** — every `DCU_API` entry in `dcu.h` returns `int` with out-params, and `DCU_ABI_VERSION` is `2`.
3. **Rewrite the managed layer** (§6): ownership and cascade disposal, weak lookup table, enqueue-only finalizers, the two-segment pump with per-subscriber isolation and snapshot iteration, the `fixed` send path, the naming and visibility changes, config validation. **Done** — `HandleTable`, `LeakTracker`, `SafeDispatch`, `Throttle` are in `Runtime/`; `SendCore` is the single `fixed` zero-copy path shared by all three `Send` overloads; cascade disposal goes through `DisposeFromParent` / `ForgetChild`.
4. **Log bridge and credential path** (§7): bounded log queue, non-detaching level changes, structured `rtc::IceServer` assignment, one-directional initialization. **Done** — `DataChannelLog` over `dcu_log_next`.
5. **Lifecycle wiring** (§6): the five domain / play-mode / quit scenarios. **Done** — `SubsystemRegistration`, `BeforeSceneLoad`, `Application.quitting`, `AssemblyReloadEvents.beforeAssemblyReload` and the Editor-only `InitializeOnLoadMethod` are all wired (`DataChannelRuntime`, `DataChannelRuntime.Editor`, `Internal/MainThread`).
6. **Tests** (§11): three assemblies, the required-contract list, the persistent domain-reload probe; delete `Assets/DataChannelVerify/`. *(The exported-symbol diff was pulled forward out of this step — it gates step 2, so it had to exist first, and it is already in place.)* **Done** — `DataChannelUnity.Tests.Editor`, `.Editor.Native` and `.Runtime` exist; `Assets/DataChannelVerify/` is gone.
7. Expand plugins. ~~Android → iOS → Win arm64 → WebGL (+ jslib)~~ — **the desktop batch (macOS universal, Windows x64, Linux x64), Android arm64-v8a and iOS arm64 are all built by CI and landed** (support table 5/5); Win arm64 left the matrix (§8). ~~iOS remains the unfinished mobile platform~~ — iOS landed via map [#90](https://github.com/xuhuanhello/juice-c-sharp/issues/90). **WebGL is the only platform still outside, and it is "if at all"**: it ships a `.a` too, but an Emscripten wasm archive, for which none of this section's Mach-O / `ld -r` / `nm` conclusions hold — see [#46](https://github.com/xuhuanhello/juice-c-sharp/issues/46), it is a different behavioural contract rather than one more toolchain.
...
9. ThirdPartyNotices + README (signaling ownership, signing, platforms). **Done** — `Packages/datachannel-unity/ThirdPartyNotices.md` ships, and the README's platform table is generated from the landed binaries rather than hand-maintained (§10).
