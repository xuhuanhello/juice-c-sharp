    ...
    public sealed class BilliardsBreakProbe : MonoBehaviour
    {
        ...
        private void OnSettled(float seconds)
        {
            ...
            Debug.Log($"[BreakProbe] settled in {seconds:F2}s | pocketed={PocketedCount} " +
                      $"clamps={ClampEvents} peakSpeed={PeakSpeed:F2}m/s " +
        ...
        {
            ClampEvents++;
            // Worth shouting about: containment held, but the rails were beaten, which means the
            // physics configuration needs looking at rather than the rules.
            Debug.LogWarning($"[BreakProbe] containment caught ball {ball.Number} leaving play.");
        }
